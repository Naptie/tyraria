<script>
  ...
  async function handleShareClicked() {
    ...
    try {
      if (
        ...
        // Ask user to save first (simplified for now)
  ...
  async function share() {
    ...
    // Call the output function with the workspace data
    onWorkspaceOutput(code);
...
{#if resourcesLoaded}
  <div class="flex flex-col h-screen w-full">
    ...
    <div class="flex-1 min-h-0">
      <Splitpanes maximizePanes={false}>
        <Pane
        ...
        <Pane size={isSidebarOpen ? 80 : 100} minSize={15}>
          <Splitpanes horizontal={isMobile} maximizePanes={false}>
            <Pane size={40}>
              <Splitpanes horizontal={true} maximizePanes={false}>
            ...
            <Pane size={40} minSize={20} maxSize={50} class="h-full">
              <TypstPreview
                class="bg-base"
                bind:this={preview}
                {reader}
